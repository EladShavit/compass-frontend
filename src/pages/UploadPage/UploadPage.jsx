...
import { parsePdfFile, guessCategory } from '../../lib/pdfParser'
...
// ── New account inline form (shown when the user has no accounts yet) ──────
    ...
    <div className={styles.newAccountCard}>
      ...
      <div className={styles.fieldGroup}>
        ...
        <input
          ...
          type="text"
          placeholder={t('upload_account_name_placeholder')}
          value={name}
      ...
      <button
        ...
        disabled={!institutionId || !name.trim() || saving}
...
// ── Step 1: Choose type + account ────────────────────────────────────────────
function StepConfigure({ docType, setDocType, accountId, setAccountId, accounts, onAccountCreated }) {
  const { t } = useLanguage()
            ...
            {parsed.map((row, i) => {
              ...
              return (
                <tr key={i} className={row.include ? '' : styles.rowExcluded}>
                  ...
                  <td>{cat?.name || '—'}</td>
                  <td style={{ textAlign: 'right', tabularNums: 'all' }}>
...
export default function UploadPage() {
  ...
  const canStep1 = docType && accountId
  const canStep2 = !!file
  ...
  // Fetch categories once for preview labels
  ...
  async function handleParse() {
    ...
    try {
      ...
      // Re-guess category using fetched category list for display
      const rows = transactions.map(tx => ({ ...tx, include: true }))
      setParsed(rows)
      setStep(3)
    } catch (err) {
      console.error(err)
  ...
  async function handleSave() {
    ...
    if (newRows.length > 0) {
      ...
      if (error) {
        ...
        insertedCount = data?.length ?? newRows.length
      }
    }
      ...
      <main className={styles.main}>
              ...
              <>
                ...
                {processing && (
                  <div className={styles.processingMsg}>
                    <LoadingSpinner />
                    <span>{t('upload_parsing_pdf')}</span>
                  </div>
                )}
              </>
            )}
        ...
        {step === 2 && (
          <FormActionsSection
            ...
            continueLabel={t('upload_parse_pdf')}
          />
        )}
        {step === 3 && (
          <FormActionsSection
